...
public class DemoTest {
    ...
    public void testCreateReportTableInfoHeader() {
        ...
        header.setEid("11738174");
        header.setName("自定义API测试");
        header.setReportCategoryId("59cf54bd59b4462841b12c85"); //报表分类Id 来源于AdminTableInfos
        ...
        String tableInfoId = openApiService.CreateReportTableInfoHeader(header, accessToken.getAccessToken());
        System.out.println(tableInfoId);
    }
    ...
    public void testSaveReportTableInfoData() {
        ...
        tableData.setEid("11738174");
        tableData.setReportCategoryId("59cf54bd59b4462841b12c85"); // 来源于AdminTableInfos,与表头保持一致
        tableData.setTableInfoId("59e01d4ab449296dae8b1a80");// 来源于表头创建或已有数据表Id
        tableData.setAddType(0);
        tableData.setData(testData);
        tableData.setBatchId("GmdGyrsuqqPqZcZRLpwzxXvjbDXu8m");// 批次id,每次传不重复随机值，建议使用uuid
    ...
    public void testGetReportTableInfoDataSaveStatus() {
        ...
        batchVo.setEid("11738174");
        batchVo.setBatchId("GmdGyrsuqqPqZcZRLpwzxXvjbDXu8q"); //数据上传使用的批次id
